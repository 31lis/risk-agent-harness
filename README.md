...
```text
.
├── evoagent/          # 核心 Python 包：API、Agent Runtime、审查、修复、存储与评测
├── skills/            # 可动态加载的代码审查 Skill
├── scripts/           # 数据导入、基准评测与提示词演化脚本
├── tests/             # 单元测试与端到端行为测试
├── web/               # Web 管理台静态资源
├── Dockerfile         # 单服务镜像
├── docker-compose.yml # PostgreSQL、Redis 与应用编排
...
项目使用 Python 3.11。克隆仓库、创建独立环境并安装运行依赖：

```powershell
git clone https://github.com/31lis/risk-agent-harness.git
Set-Location risk-agent-harness
python -m venv .venv
.\.venv\Scripts\Activate.ps1
python -m pip install -r requirements.txt
```

然后在同一个 PowerShell 窗口中配置本地管理员并启动服务：
...
也可以复制 `.env.example` 为 `.env`，填入随机的登录密钥、管理员密码及所选模型的 API Key。`.env` 已被忽略，不会进入 Git 历史。

使用 Docker Compose 时，必须先在 `.env` 中设置 `EVOAGENT_AUTH_SECRET` 和 `EVOAGENT_BOOTSTRAP_ADMIN_PASSWORD`，随后运行：
...
发现安全问题时，请不要在公开 Issue 中披露凭据或可直接利用的细节；处理建议见 [SECURITY.md](SECURITY.md)。项目采用 [MIT License](LICENSE)。
